...
function check1(btn) {
  ...
  // if( Number(btn.innerText)) == NaN
  if (isNaN(Number(btn.innerText))) {
    btn.innerText = 0;

  } else {
    // ++ 증감 연산자는 오로지number 타입에만 사용가능!
    // 피연산자가 숫자형태의 문자열인 경우 ( "123" )
    // -> 피연산자를 강제로 숫자 타입을 바꿔서 연산을 수행
    btn.innerText++
  }
}
...
test1c.onclick = function(){
  test1c.style.backgroudColor = "skyblue";
};
...
/* 작성법
  ...
  *이벤트 종류 : click, change, sumbit, keydown
  *이벤트 핸들러 : 감지되었을떄 수행할 함수
*/
...
// - 요소.addEventListner()는 요소에만 사용 가능!!
const btns 
