...
function check1() {
  ...
  // id 가 "input1" 인 요소를 얻어와 input1 변수에 저장
  const input1 = document.getElementById("input1");
  // input1.value : input1 요소에 입력된 값 (string)
  //  number(문자열은 > 숫자로 변경)
  const value = Number(input1.value);
  // 입력받은 값이 0보다 큰 경우는 true(라는 조건식 장성)
  if (value > 0) {
    alert(`${value}은는 양수입니다.`);

  }

  // 입력받은 값이 0보다 작거나 같은 경우
  //  == 양수가 아닌 경우

  if (value <= 0) {
    alert(`${value}은는 음수입니다.`);
  }
}
...
function check2() {
  ...
  // 0~99 사이 난수 발생
  const randomNumber
    = Math.floor(Math.random() * 100);  /* 100곱하기 */


  if (randomNumber % 2 === 0) {
    alert(`${randomNumber} 은/는 짝수입니다.`);
  }
  else { /* 조건식이 false 인 경우 == 홀수인 경우 */
...
// function ageCheck(){
//   input1 = document.getElementById("inputAge");
//   const value = Number(input1.value);

//   if(value < 0){
//     alert (`${input1} 잘못 입력 하셨습니다`);
//   }
//   else if (value <= 13){
//     alert (`${input1} 어린이 입니다`);

//   }
//   else if (value <= 19){
//    alert (`${input1} 청소년 입니다`);
//   }
...
function check5(){
  ...
  // 1) A와B 중 하나라도 입력되지 않으면
  // -> 입력되지 않는 값이 존재합니다.

  if (a.value.length === 0 || b.value.length === 0) {
    alert("입력되지 않은 값이 존재합니다.")
    return; //Early Retrun : 함수를 여기서 일찍 끝냄
  }

  // 두 값이 모두 입력되어있을 경우에만 아래코드 실행
...
function calc(op){
  ...
  let result;  /* 결과저장 */
  // op 값에 따라 알맞은 case 실행 /* break : 멈추다 */
/*   ->break 미작정시 
            현재 case 수행 멈추지 않고 다음 break가 존재하는 default 까지 수행*/ 
  switch(op){
