...
document.querySelector("#btn3")
  .addEventListener("click", function () {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      //  3) numberArr의 i번쨰 인덱스 요소에 num 저장
...
function pow(num, x) {
  ...
  let result = 1; // 곱할떄는 영향을 주지않는 1 로 곱하기
...
//임력 함수 확인
const btm6 = document.querySelector("#btn6");


//익명 함수를 이벤트 핸들러로 많이 사용하는 이유
// -> 해당 이벤트 전용 그닝인 경우가 많아서
//  (다른곳에서는 재사용을 못하는 경우가 많아서)
btn6.addEventListener("click", function () { //익명함수
  ...
  // 변수명이 익명 함수의 함수명 역활을 해준다
  // (요즘 하무 정의 방법 (트렌드다))


  // 해당 함수는 지역 변수에 저장됨 -> 이 지역에서만,
  //  -> 해당 지점에서만 사용 가능한 함수
...
arrows[2].addEventListener("click", () => {
  ...
  // 함수 정의 부분이 return한 줄 --> {}생략이 가능
  const minusFn = (a, b) => a - b;

  alert(minusFn (10, 3));

  // 매개 변수 1개, 정의 내용 return 한줄
