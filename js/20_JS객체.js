...
function Student(inputGrade, inputBan, inputNumber, inputName) {
  ...
  // 필드(객체의 변수)
  this.grade = inputGrade; // 전달받은 inputGrade를
...
addStudent.addEventListener("click", () => {
  ...
  const std = new Student(inputGrade.value,
    inputBan.value,
    inputNumber.value,
    inputName.value);
  ...
  // 이전 입력 값 삭제
  inputGrade.value = '';  // 빈 문자열을 value에 대입에 삭제한 효과
  inputBan.value = '';
  inputNumber.value = '';
  inputName.value = '';
});
...
printStudent.addEventListener("click", ()=>{
  ...
  // 이전 #result 에 작성되어있는 내용 삭제

  result.innerHTML = '';

  // studentList에 존재하는 모든 학생 객체 얻어오기 접근하기
  for(let i = 0 ; i < studentList.length ; i++){
    // 학생 정보를 한 줄로 반환하는 메서드 호출
    const info = studentList[i].informtaion()
    // li 태그 생성하는법
...
checkJSON.addEventListener("click", () => {
  ...
  // JSON 생성 (JS 개체 모양의 문자열)
  const json2 = '{"menu" : "김밥", "price" : 400}'
  ...
  console.log(obj2);
   ...
   // 기상청 서버에 7/12 오전 10 전국 미세먼지 농도를 JSON으로 받아오기
   fetch('https://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getMinuDustFrcstDspth?serviceKey=BTjX5Rqk5SZSwrW687YxxqoqoH7FbYV/BKqfde1PPn0jiIoOy6aYAUb1MuK7h9izWzM/YX6SVOjBBUMIuwRRIg==&returnType=JSON&numOfRows=100&pageNo=1&searchDate=2024-07-12&InformCode=AM10')
